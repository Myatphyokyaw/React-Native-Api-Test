...
import { View, StyleSheet, Text, Image, TouchableOpacity, TouchableNativeFeedback, Modal } from "react-native";
import { useNavigation } from "@react-navigation/native";
import { Avatar, IconButton, Portal, Provider } from "react-native-paper";
...
const Feeds = props => {
  ...
  useEffect(() => {
    if (props.data.feature_img === null) {
      setIsExit(false);
    } else {
      setIsExit(true);
    }
  ...
  const containerStyle = { backgroundColor: "black", padding: 20, flex: 1 };
  ...
  return (
    <View style={styles.card}>
      ...
      <Modal transparent={true} visible={visible} contentContainerStyle={containerStyle}>
        <View>

        </View>
      </Modal>
      <TouchableOpacity activeOpacity={.9} onPress={showModal}>
        <Image style={isExit ? styles.photo : styles.noPhoto} source={{ uri: props.data.feature_img }} />
      </TouchableOpacity>
      ...
      <View style={styles.bottomSection}>
        <View style={{ flexDirection: "row" }}>
          <LottieView style={styles.heartReaction} source={require("../assests/animation/like.json")} autoPlay loop />
          <TouchableOpacity style={styles.reactionBtn}>
            <Fontisto color={COLORS.black} name="comment" size={20} />
          </TouchableOpacity>
          <TouchableOpacity style={styles.reactionBtn}>
            <Ionicons color={COLORS.black} name="share-social-outline" size={25} />
          </TouchableOpacity>
        </View>
        <TouchableNativeFeedback onPress={() => navigation.navigate("Detail", { data: props.data })}>
